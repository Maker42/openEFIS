...
class TakeoffControlVTOL(FileConfig.FileConfig):
    def __init__(self, att_cont, middle_engine_tilt, outer_engine_release, sensors, callback):
        ...
        self.ClimbRateLimits = (-100.0, 100.0)        # feet / minute
        ...
        self.MaxGroundSpeed = 20.0              # Max speed to correct position
        self._PitchPID = None
        self._RollPID = None
        ...
        self._outer_engine_release = outer_engine_release
    ...
    def Update(self):
        ...
        if (self._flight_mode == SUBMODE_ASCEND or
            ...
            self.GroundTrack = self._sensors.GroundTrack()
            self.CurrentGroundSpeed = self._sensors.GroundSpeed()
            relative_heading = self.GroundTrack - self.CurrentHeading
            current_forward_speed = self.CurrentGroundSpeed * math.cos(relative_heading)
            current_side_speed = self.CurrentGroundSpeed * math.sin(relative_heading)
            ...
            middle_engine_roll_contribution = math.sin(self.TransitionSteps[self._transition_step][0])
            if self._sensors.AirSpeed() >= self.TransitionSteps[self._transition_step][1]:
                ...
                if self._transition_step >= len(self.TransitionSteps):
                    ...
                    self._outer_engine_release.Set(1)
            ...
            if self._sensors.OuterEnginePosition() == "forward":
                self._outer_engine_release.Set(0)
