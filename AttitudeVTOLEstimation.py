...
def UpdatePosition(pos, _time, direction, speed):
    pos.x += speed * _time * math.sin (direction * util.RAD_DEG)
    pos.y += speed * _time * math.cos (direction * util.RAD_DEG)
...
PITCH_INCREMENTS=[(.25, 5.0), (0.5, 5.0), (1.0, 5.0), (2.0, 10.0), (3.0, 15.0), (5.0, 15.0)]
...
class AttitudeVTOLEstimation:
    ...
    def EstimateNextAttitude(self,
        ...
        if current_position != self.last_position:
            ...
            if time_delta > 0.0:
                ...
                UpdatePosition(self.position_offset, time_delta, ground_track, current_speed / (60.0 * 3600))
        ...
        if self.next_change_time == 0 or _time >= self.next_change_time:
            ...
            desired_groundspeed = util.get_rate(0.0, distance * 60.0, MinutesPositionCorrection / 60.0,
                    MaxGroundSpeed)
            ...
            if vector_time >= 2.0:
                ...
                logger.debug ("thrust_angle = %g, relative_thrust_theta = %g, pitch = %g, roll = %g",
    ...
    def EstimateWindForce(self, airplane, sensors, current_true_heading, current_velocity):
        ...
        wind_force.x *= abs(coefx)
        wind_force.y *= abs(coefy)
